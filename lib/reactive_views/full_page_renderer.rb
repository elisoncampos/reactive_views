  ...
  class FullPageRenderer
    class << self
      ...
      def render(controller, template_full_path:)
        ...
        # Step 2: Write TSX to temporary file
        temp_tsx_path = write_temp_tsx(tsx_content)

        # Step 3: Build props from instance variables + explicit props
        props = build_props(controller, tsx_content)

        # Step 4: SSR the TSX file
        html = Renderer.render_path(temp_tsx_path, props)

        html
      ensure
        # Clean up temp file
        File.delete(temp_tsx_path) if temp_tsx_path && File.exist?(temp_tsx_path)
      end
      ...
      def write_temp_tsx(tsx_content)
        ...
        temp_path = File.join(temp_dir, "page_#{timestamp}_#{random}.tsx")

        File.write(temp_path, tsx_content)
        temp_path
      end

      def build_props(controller, tsx_content)
        ...
        inferred_keys = PropsInference.infer_props(tsx_content)
