...
module ReactiveViews
  class Railtie < Rails::Railtie
    ...
    initializer "reactive_views.full_page_fallback", after: :load_config_initializers, before: "reactive_views.transform_components" do
      ActiveSupport.on_load(:action_controller) do
        ...
        ActionController::Base.class_eval do
          def default_render(*args)
            ...
            begin
              super
            rescue ActionView::MissingTemplate, ActionController::UnknownFormat, ActionController::MissingExactTemplate => exception
              # Only handle if full_page_enabled and HTML request
              unless ReactiveViews.config.full_page_enabled && request.format&.html?
                raise exception
              end

              Rails.logger.debug("[ReactiveViews] MissingTemplate caught in default_render: #{exception.message}") if Rails.logger

              # Check for .tsx.erb fallback template
              template_path = Rails.root.join("app", "views", controller_path, "#{action_name}.tsx.erb")
              Rails.logger.debug("[ReactiveViews] Looking for fallback template at: #{template_path}") if Rails.logger

              if File.exist?(template_path)
                Rails.logger.debug("[ReactiveViews] Rendering full-page TSX.ERB") if Rails.logger
                begin
                  html = ReactiveViews::FullPageRenderer.render(self, template_full_path: template_path)
                  Rails.logger.debug("[ReactiveViews] Full-page rendered #{html.length} bytes") if Rails.logger
                  self.response_body = html
                  self.status = 200
                  self.content_type ||= "text/html"
                rescue => e
                  Rails.logger.error("[ReactiveViews] Full-page render error: #{e.class}: #{e.message}") if Rails.logger
                  Rails.logger.error(e.backtrace.join("\n")) if Rails.logger
                  raise exception
                end
              else
                Rails.logger.debug("[ReactiveViews] TSX.ERB template not found, re-raising") if Rails.logger
                raise exception
              end
            end
          end
